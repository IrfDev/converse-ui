...
const BOOTSTRAP_IGNORE_MODULES = (process.env.BOOTSTRAP_IGNORE_MODULES || '')
  .replace(/ /g, '')
  .trim();
if (BOOTSTRAP_IGNORE_MODULES.length > 0) {
  bootstrap_ignore_modules = bootstrap_ignore_modules.concat(
    BOOTSTRAP_IGNORE_MODULES.split(',')
  );
}
...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      ...
      SecondLevelDomains: path.resolve(
        __dirname,
        'node_modules/urijs/src/SecondLevelDomains'
      ),
      'formdata-polyfill': path.resolve(
        __dirname,
