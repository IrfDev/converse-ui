...
import { createStore } from '@converse/staytus/shared/utils.js';
...
export function extendAppSettings(settings) {
  ...
  const allowed_keys = Object.keys(
    pick(settings, Object.keys(DEFAULT_SETTINGS))
  );
  const allowed_site_settings = pick(init_settings, allowed_keys);
  const updated_settings = assignIn(
    pick(settings, allowed_keys),
    allowed_site_settings
  );
