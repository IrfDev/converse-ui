...
export class Connection extends Strophe.Connection {
  ...
  async onDomainDiscovered(response) {
    ...
    const xrd = new window.DOMParser().parseFromString(text, 'text/xml')
      .firstElementChild;
    if (
      xrd.nodeName != 'XRD' ||
      xrd.namespaceURI != 'http://docs.oasis-open.org/ns/xri/xrd-1.0'
    ) {
      return log.warn('Could not discover XEP-0156 connection methods');
    }
    const bosh_links = sizzle(
      `Link[rel="urn:xmpp:alt-connections:xbosh"]`,
      xrd
    );
    const ws_links = sizzle(
  ...
  async onConnected(reconnecting) {
     ...
     */
    await api.trigger('afterResourceBinding', reconnecting, {
      synchronous: true,
    });
  ...
  onConnectStatusChanged(status, message) {
    ...
    if (status === Strophe.Status.ATTACHFAIL) {
      ...
      if (
        this.worker_attach_promise?.isResolved &&
        this.status === Strophe.Status.ATTACHED
      ) {
      ...
      if (this.reconnecting) {
        log.debug(
          status === Strophe.Status.CONNECTED ? 'Reconnected' : 'Reattached'
        );
        this.onConnected(true);
      } else {
        log.debug(
          status === Strophe.Status.CONNECTED ? 'Connected' : 'Attached'
        );
        if (this.restored) {
      ...
      if (message === 'host-unknown' || message == 'remote-connection-failed') {
        ...
        message !== undefined &&
        message === Strophe?.ErrorCondition?.NO_AUTH_MECH
      ) {
        feedback = __(
  ...
  hasResumed() {
    if (api.settings.get('connection_options')?.worker || this.isType('bosh')) {
      return (
        _converse.connfeedback.get('connection_status') ===
