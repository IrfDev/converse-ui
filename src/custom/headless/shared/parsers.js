...
import { _converse, api } from '@converse/staytus/core';
import { decodeHTMLEntities } from 'shared/utils';
import { rejectMessage } from '@converse/staytus/shared/actions';
...
export function getStanzaIDs(stanza, original_stanza) {
  ...
  const result = sizzle(
    `message > result[xmlns="${Strophe.NS.MAM}"]`,
    original_stanza
  ).pop();
  if (result) {
  ...
  // Store the origin id
  const origin_id = sizzle(
    `origin-id[xmlns="${Strophe.NS.SID}"]`,
    stanza
  ).pop();
  if (origin_id) {
...
export function getRetractionAttributes(stanza, original_stanza) {
  ...
  if (fastening) {
    ...
    if (retracted) {
      ...
      const time = delay
        ? dayjs(delay.getAttribute('stamp')).toISOString()
...
export function getCorrectionAttributes(stanza, original_stanza) {
  const el = sizzle(
    `replace[xmlns="${Strophe.NS.MESSAGE_CORRECT}"]`,
    stanza
  ).pop();
  if (el) {
...
export function getSpoilerAttributes(stanza) {
  const spoiler = sizzle(
    `spoiler[xmlns="${Strophe.NS.SPOILER}"]`,
    stanza
  ).pop();
  return {
    is_spoiler: !!spoiler,
...
export function getReferences(stanza) {
  ...
  return sizzle(`reference[xmlns="${Strophe.NS.REFERENCE}"]`, stanza).map(
    (ref) => {
...
export function getReceiptId(stanza) {
  const receipt = sizzle(
    `received[xmlns="${Strophe.NS.RECEIPTS}"]`,
    stanza
  ).pop();
...
export function rejectUnencapsulatedForward(stanza) {
  const bare_forward = sizzle(
    `message > forwarded[xmlns="${Strophe.NS.FORWARD}"]`,
    stanza
  ).length;
  if (bare_forward) {
    rejectMessage(
    ...
    return new StanzaParseError(
      `Ignoring unencapsulated forwarded message from ${from_jid}`,
      stanza
    );
  }
}
