...
import _converse from '@converse/staytus/shared/_converse';
import advancedFormat from 'dayjs/plugin/advancedFormat';
import dayjs from 'dayjs';
import debounce from 'lodash/debounce';
import i18n from '@converse/staytus/shared/i18n';
...
import u from '@converse/staytus/utils/core';
import { Collection } from '@converse/skeletor/src/collection';
import {
  Connection,
  MockConnection,
} from '@converse/staytus/shared/connection.js';
import {
  clearUserSettings,
  extendAppSettings,
  getAppSetting,
  ...
  updateAppSettings,
  updateUserSettings,
} from '@converse/staytus/shared/settings';
import { Events } from '@converse/skeletor/src/events.js';
import { Model } from '@converse/skeletor/src/model.js';
import { Strophe, $build, $iq, $msg, $pres } from 'strophe.js/src/strophe';
import { TimeoutError } from '@converse/staytus/shared/errors';
import { createStore, replacePromise } from '@converse/staytus/shared/utils';
...
// These are just the @converse/staytus plugins, for the full converse,
...
export const api = (_converse.api = {
  ...
  hook(name, context, data) {
    ...
    if (events.length) {
      ...
      // sent to this hook.
      const o = events.reduce(
        (o, e) => o.then((d) => e.callback(context, d)),
  ...
  user: {
    ...
    async login(jid, password, automatic = false) {
      ...
      if (
        ...
        (jid && !u.isSameDomain(_converse.connection.jid, jid))
      ) {
        await _converse.initConnection();
      }
      if (
        api.settings.get('connection_options')?.worker &&
        (await _converse.connection.restoreWorkerSession())
      ) {
        return;
      }
      if (jid) {
    ...
    logout() {
      ...
      _converse.connection.setDisconnectionCause(
        _converse.LOGOUT,
        undefined,
        true
      );
    ...
    settings: {
      ...
      async get(key, fallback) {
        ...
        return user_settings.get(key) === undefined
          ? fallback
    ...
    if (['get', 'set'].includes(stanza.getAttribute('type'))) {
      ...
      if (reject) {
        ...
        promise.catch((e) => {
          if (e === null) {
            throw new TimeoutError(
              `Timeout error after ${timeout}ms for the following IQ stanza: ${Strophe.serialize(
                stanza
              )}`
            );
          }
        ...
        promise = new Promise((resolve) =>
          _converse.connection.sendIQ(stanza, resolve, resolve, timeout)
        );
      }
    } else {
...
_converse.isUniView = function () {
   ...
   */
  return ['mobile', 'fullscreen', 'embedded'].includes(
    api.settings.get('view_mode')
  );
};
...
function initPersistentStorage() {
  if (api.settings.get('persistent_store') === 'sessionStorage') {
    ...
    _converse.api.settings.get('persistent_store') === 'BrowserExtLocal'
  ) {
    Storage.localForage
    ...
    _converse.api.settings.get('persistent_store') === 'BrowserExtSync'
  ) {
    Storage.localForage
...
_converse.getDefaultStore = function () {
  if (_converse.config.get('trusted')) {
    const is_non_persistent =
      api.settings.get('persistent_store') === 'sessionStorage';
...
function initPlugins() {
  ...
  const whitelist = CORE_PLUGINS.concat(
    _converse.api.settings.get('whitelisted_plugins')
  );
...
async function attemptNonPreboundSession(credentials, automatic) {
  ...
  if (api.settings.get('authentication') === _converse.LOGIN) {
    ...
    if (credentials) {
      ...
      _converse.jid &&
      (_converse.api.settings.get('password') || _converse.connection.pass)
    ) {
      connect();
      ...
      !_converse.isTestEnv() &&
        log.warn(
          "attemptNonPreboundSession: Couldn't find credentials to log in with"
        );
    }
  } else if (
    [_converse.ANONYMOUS, _converse.EXTERNAL].includes(
      _converse.api.settings.get('authentication')
    ) &&
...
async function initSession(jid) {
  ...
  if (_converse.session?.get('id') !== id) {
    ...
    _converse.session.browserStorage = createStore(
      id,
      is_shared_session ? 'persistent' : 'session'
    );
...
function saveJIDtoSession(jid) {
  ...
  if (
    _converse.api.settings.get('authentication') !== _converse.ANONYMOUS &&
...
function setUpXMLLogging() {
  ...
  _converse.connection.xmlInput = (body) =>
    log.debug(body.outerHTML, 'color: darkgoldenrod');
  _converse.connection.xmlOutput = (body) =>
...
_converse.ConnectionFeedback = Model.extend({
  ...
  initialize() {
    this.on('change', () =>
      api.trigger('connfeedback', _converse.connfeedback)
    );
  },
});
...
Object.assign(converse, {
  ...
  async initialize(settings) {
    ...
    _converse.router.route(
      /^converse\?loglevel=(debug|info|warn|error|fatal)$/,
      'loglevel',
      (l) => log.setLogLevel(l)
    );
    ...
    if (api.settings.get('idle_presence_timeout') > 0) {
      api.listen.on('addClientFeatures', () =>
        api.disco.own.features.add(Strophe.NS.IDLE)
      );
    }
    ...
    if (
      ...
      (api.settings.get('keepalive') &&
        invoke(plugins['converse-bosh'], 'enabled'))
    ) {
