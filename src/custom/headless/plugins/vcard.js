...
converse.plugins.add('converse-vcard', {
  ...
  initialize() {
    ...
    async function getVCard(_converse, jid) {
      const to =
        Strophe.getBareJidFromJid(jid) === _converse.bare_jid ? null : jid;
      let iq;
      try {
    ...
    function getVCardForChatroomOccupant(message) {
      ...
      if (chatbox && chatbox.get('nick') === nick) {
        ...
        const jid =
          (message.occupant && message.occupant.get('jid')) ||
          message.get('from');
        if (jid) {
          return (
            _converse.vcards.findWhere({ jid }) ||
            _converse.vcards.create({ jid })
          );
        } else {
          log.error(
            `Could not assign VCard for message because no JID found! msgid: ${message.get(
              'msgid'
            )}`
          );
          return;
    ...
    _converse.initVCardCollection = async function () {
      ...
      _converse.vcards.browserStorage = _converse.createStore(
        `${_converse.bare_jid}-converse.vcards`
      );
      ...
      if (_converse.session) {
        ...
        _converse.xmppstatus.vcard =
          vcards.findWhere({ jid: jid }) || vcards.create({ jid: jid });
      }
      /**
    ...
    Object.assign(_converse.api, {
      ...
      vcard: {
        ...
        async update(model, force) {
          ...
          model =
            typeof model === 'string'
              ? _converse.vcards.findWhere({ jid: model })
              : model;
          if (!model) {
