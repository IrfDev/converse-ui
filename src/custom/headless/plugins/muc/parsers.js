...
} from '@converse/staytus/shared/parsers';
import { api, converse } from '@converse/staytus/core';
...
function getModerationAttributes(stanza) {
  const fastening = sizzle(
    `apply-to[xmlns="${Strophe.NS.FASTEN}"]`,
    stanza
  ).pop();
  if (fastening) {
    const applies_to_id = fastening.getAttribute('id');
    const moderated = sizzle(
      `moderated[xmlns="${Strophe.NS.MODERATE}"]`,
      fastening
    ).pop();
    if (moderated) {
    ...
    const tombstone = sizzle(
      `> moderated[xmlns="${Strophe.NS.MODERATE}"]`,
      stanza
    ).pop();
    if (tombstone) {
      const retracted = sizzle(
        `retracted[xmlns="${Strophe.NS.RETRACT}"]`,
        tombstone
      ).pop();
      if (retracted) {
        return {
...
export async function parseMUCMessage(stanza, chatbox, _converse) {
  ...
  if (
    sizzle(`message > forwarded[xmlns="${Strophe.NS.FORWARD}"]`, stanza).length
  ) {
    return new StanzaParseError(
      `Invalid Stanza: Forged MAM groupchat message from ${stanza.getAttribute(
        'from'
      )}`,
      stanza
    );
  ...
  let attrs = Object.assign(
    {
      ...
      is_markable: !!sizzle(`markable[xmlns="${Strophe.NS.MARKERS}"]`, stanza)
        .length,
      is_marker: !!marker,
      is_unstyled: !!sizzle(`unstyled[xmlns="${Strophe.NS.STYLING}"]`, stanza)
        .length,
  ...
  if (
    attrs.is_archived &&
    original_stanza.getAttribute('from') !== attrs.from_muc
  ) {
    return new StanzaParseError(
      `Invalid Stanza: Forged MAM message from ${original_stanza.getAttribute(
        'from'
      )}`,
      stanza
    );
    ...
    attrs.is_archived &&
    original_stanza.getAttribute('from') !== chatbox.get('jid')
  ) {
    return new StanzaParseError(
      `Invalid Stanza: Forged MAM groupchat message from ${stanza.getAttribute(
        'from'
  ...
  attrs['id'] =
    attrs['origin_id'] ||
    attrs[`stanza_id ${attrs.from_muc || attrs.from}`] ||
    u.getUniqueId();
  /**
...
export function parseMUCPresence(stanza) {
  ...
  Array.from(stanza.children).forEach((child) => {
    if (child.matches('status')) {
      ...
      child.matches('x') &&
      child.getAttribute('xmlns') === Strophe.NS.MUC_USER
    ) {
      ...
      child.matches('x') &&
      child.getAttribute('xmlns') === Strophe.NS.VCARDUPDATE
    ) {
      data.image_hash = child.querySelector('photo')?.textContent;
    } else if (
      child.matches('hats') &&
      child.getAttribute('xmlns') === Strophe.NS.MUC_HATS
    ) {
