...
function disconnectChatRooms() {
  ...
  return _converse.chatboxes
    ...
    .forEach((m) =>
      m.session.save({ connection_status: converse.ROOMSTATUS.DISCONNECTED })
    );
}
...
converse.plugins.add('converse-muc', {
  ...
  initialize() {
    ...
    if (
      api.settings.get('locked_muc_domain') &&
      typeof api.settings.get('muc_domain') !== 'string'
    ) {
      throw new Error(
    ...
    _converse.onDirectMUCInvitation = async function (message) {
      ...
      if (api.settings.get('auto_join_on_invite')) {
        ...
        if (!reason) {
          result = confirm(
            __(
              '%1$s has invited you to join a groupchat: %2$s',
              contact,
              room_jid
            )
          );
      ...
      if (result === true) {
        ...
        if (
          chatroom.session.get('connection_status') ===
    ...
    api.listen.on('beforeResourceBinding', () => {
      _converse.connection.addHandler(
        (stanza) => {
          const muc_jid = Strophe.getBareJidFromJid(
