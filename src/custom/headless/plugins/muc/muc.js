...
import {
  ...
  setAffiliations,
  getAffiliationList,
} from './affiliations/utils.js';
import { isArchived } from '@converse/staytus/shared/parsers';
import { parseMUCMessage, parseMUCPresence } from './parsers.js';
import { sendMarker } from '@converse/staytus/shared/actions';
...
const ChatRoomMixin = {
  ...
  async initialize() {
    ...
    this.listenTo(
      this.occupants,
      'change:affiliation',
      this.createAffiliationChangeMessage
    );
  ...
  enableRAI() {
    if (api.settings.get('muc_subscribe_to_rai')) {
      ...
      api.user.presence.send(
        ...
        muc_domain,
        null,
        $build('rai', { xmlns: Strophe.NS.RAI })
      );
    }
  },
  ...
  onOccupantAdded(occupant) {
    if (
      ...
      this.updateNotifications(
        occupant.get('nick'),
        converse.MUC_TRAFFIC_STATES.ENTERED
      );
    }
  },
  ...
  onOccupantRemoved(occupant) {
    if (
      ...
      this.updateNotifications(
        occupant.get('nick'),
        converse.MUC_TRAFFIC_STATES.EXITED
      );
    }
  },
  ...
  handleMessageFromMUCHost(stanza) {
    ...
    const active_mucs = Array.from(rai?.querySelectorAll('activity') || []).map(
      (m) => m.textContent
    );
  ...
  handleForwardedMentions(stanza) {
    ...
    const mentions = msgs.filter(
      (m) => Strophe.getBareJidFromJid(m.getAttribute('from')) === muc_jid
    );
    if (mentions.length) {
      this.save({
        has_activity: true,
  ...
  removeHandlers() {
    ...
    if (this.domain_message_handler) {
      _converse.connection &&
        _converse.connection.deleteHandler(this.domain_message_handler);
    ...
    if (this.domain_presence_handler) {
      _converse.connection &&
        _converse.connection.deleteHandler(this.domain_presence_handler);
  ...
  canModerateMessages() {
    ...
    return (
      self &&
      self.isModerator() &&
      api.disco.supports(Strophe.NS.MODERATE, this.get('jid'))
    );
  },
  ...
  parseTextForReferences(text) {
    ...
    const uriFromNickname = (nickname) => {
      ...
      const uri =
        (this.features.get('nonanonymous') && occupant?.get('jid')) ||
        `${jid}/${nickname}`;
  ...
  getAllowedCommands() {
    ...
    if (
      this.config.get('changesubject') ||
      ['owner', 'admin'].includes(this.getOwnAffiliation())
    ) {
    ...
    if (this.verifyAffiliations(['owner'], occupant, false)) {
      allowed_commands = allowed_commands
        .concat(OWNER_COMMANDS)
        .concat(ADMIN_COMMANDS);
    ...
    if (Array.isArray(api.settings.get('muc_disable_slash_commands'))) {
      return allowed_commands.filter(
        (c) => !api.settings.get('muc_disable_slash_commands').includes(c)
      );
    } else {
  ...
  verifyAffiliations(affiliations, occupant, show_error = true) {
    ...
    occupant =
      occupant || this.occupants.findWhere({ jid: _converse.bare_jid });
    if (occupant) {
  ...
  async updateMemberLists(members) {
    ...
    const aff_lists = await Promise.all(
      all_affiliations.map((a) => getAffiliationList(a, muc_jid))
    );
    ...
    await setAffiliations(
      muc_jid,
      computeAffiliationsDelta(true, false, members, old_members)
    );
    await this.occupants.fetchMembers();
  },
  ...
  async shouldShowErrorMessage(attrs) {
    if (
      attrs['error_condition'] === 'not-acceptable' &&
  ...
  getNotificationsText() {
    ...
    const role_changes = api.settings
      ...
      .filter((role_change) =>
        converse.MUC_ROLE_CHANGES_LIST.includes(role_change)
      );

    const join_leave_events = api.settings
      .get('muc_show_info_messages')
      .filter((join_leave_event) =>
        converse.MUC_TRAFFIC_STATES_LIST.includes(join_leave_event)
      );
    ...
    return states.reduce((result, state) => {
      ...
      const actors = existing_actors.map(
        (a) => this.getOccupant(a)?.getDisplayName() || a
      );
      if (actors.length === 1) {
        if (state === 'composing') {
        ...
        if (state === 'composing') {
          ...
          return `${result}${__(
            '%1$s have entered the groupchat',
            actors_str
          )}\n`;
        } else if (state === 'exited') {
  ...
  updateNotifications(actor, state) {
    ...
    const actors_per_traffic_state = converse.MUC_TRAFFIC_STATES_LIST.reduce(
      reducer,
      {}
    );
    const actors_per_role_change = converse.MUC_ROLE_CHANGES_LIST.reduce(
      reducer,
      {}
    );
  ...
  async onMessage(attrs) {
    ...
    if (message) {
      ...
      attrs.is_valid_receipt_request ||
      attrs.is_marker ||
    ...
    if (u.shouldCreateGroupchatMessage(attrs)) {
      const msg =
        this.handleCorrection(attrs) || (await this.createMessage(attrs));
      this.removeNotification(attrs.nick, ['composing', 'paused']);
  ...
  handleDisconnection(stanza) {
    ...
    const actor = item
      ? invoke(item.querySelector('actor'), 'getAttribute', 'nick')
      : undefined;
    const message = _converse.muc.disconnect_messages[codes[0]];
    const status = codes.includes('301')
  ...
  getActionInfoMessage(code, nick, actor) {
    ...
    if (code === '301') {
      return actor
        ? __('%1$s has been banned by %2$s', nick, actor)
        : __('%1$s has been banned', nick);
    } else if (code === '303') {
      return __("%1$s's nickname has changed", nick);
    } else if (code === '307') {
      return actor
        ? __('%1$s has been kicked out by %2$s', nick, actor)
  ...
  createAffiliationChangeMessage(occupant) {
    ...
    if (
      ...
      (current_affiliation === 'admin' &&
        _converse.isInfoVisible(converse.AFFILIATION_CHANGES.ADMIN)) ||
      (current_affiliation == 'owner' &&
        _converse.isInfoVisible(converse.AFFILIATION_CHANGES.OWNER))
    ) {
  ...
  createRoleChangeMessage(occupant, changed) {
    ...
    if (
      ...
      _converse.isInfoVisible(converse.MUC_ROLE_CHANGES.DEOP)
    ) {
      this.updateNotifications(
        occupant.get('nick'),
        converse.MUC_ROLE_CHANGES.DEOP
      );
    } else if (
      previous_role === 'visitor' &&
      _converse.isInfoVisible(converse.MUC_ROLE_CHANGES.VOICE)
    ) {
      this.updateNotifications(
        occupant.get('nick'),
        converse.MUC_ROLE_CHANGES.VOICE
      );
    }
    if (
      occupant.get('role') === 'visitor' &&
      _converse.isInfoVisible(converse.MUC_ROLE_CHANGES.MUTE)
    ) {
      this.updateNotifications(
        occupant.get('nick'),
        converse.MUC_ROLE_CHANGES.MUTE
      );
  ...
  createInfoMessage(code, stanza, is_self) {
    ...
    if (code === '110' || (code === '100' && !is_self)) {
      ...
      const item = stanza.querySelector(
        `x[xmlns="${Strophe.NS.MUC_USER}"] item`
      );
      data.actor = item
        ? item.querySelector('actor')?.getAttribute('nick')
        : undefined;
      data.reason = item
        ? item.querySelector('reason')?.textContent
        : undefined;
  ...
  createInfoMessages(stanza) {
    const codes = sizzle(
      `x[xmlns="${Strophe.NS.MUC_USER}"] status`,
      stanza
    ).map((s) => s.getAttribute('code'));
  ...
  setDisconnectionState(
    ...
    reason,
    actor,
    status = converse.ROOMSTATUS.DISCONNECTED
  ) {
    this.save({
  ...
  onPresenceFromMUCHost(stanza) {
    if (stanza.getAttribute('type') === 'error') {
      const error = stanza.querySelector('error');
      if (
        error?.getAttribute('type') === 'wait' &&
        error?.querySelector('resource-constraint')
      ) {
        ...
        if (
          this.session.get('connection_status') ===
          converse.ROOMSTATUS.DISCONNECTED
        ) {
          this.rejoin();
        }
  ...
  clearUnreadMsgCounter() {
    if (
      this.get('num_unread_general') > 0 ||
      this.get('num_unread') > 0 ||
      this.get('has_activity')
    ) {
