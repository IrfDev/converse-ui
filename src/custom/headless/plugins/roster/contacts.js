...
const RosterContacts = Collection.extend({
    ...
    _converse.connection.addHandler(
      function (msg) {
        window.setTimeout(function () {
          ...
          _converse.roster.subscribeToSuggestedItems.bind(_converse.roster)(
            msg
          );
        }, t);
      ...
      if (item.getAttribute('action') === 'add') {
        _converse.roster.addAndSubscribe(
          ...
          _converse.xmppstatus.getNickname() ||
            _converse.xmppstatus.getFullname()
        );
      }
    });
    return true;
  ...
  async subscribeBack(bare_jid, presence) {
    ...
    if (contact instanceof _converse.RosterContact) {
      ...
      const nickname =
        sizzle(`nick[xmlns="${Strophe.NS.NICK}"]`, presence).pop()
          ?.textContent || null;
  ...
  getNumOnlineContacts() {
    ...
    return sum(
      this.models.filter((m) => !ignored.includes(m.presence.get('show')))
    );
  },
  ...
  onRosterPush(iq) {
    ...
    if (from && from !== _converse.bare_jid) {
      ...
      log.warn(
        `Ignoring roster illegitimate roster push message from ${iq.getAttribute(
          'from'
        )}`
      );
      return;
    ...
    if (items.length > 1) {
      log.error(iq);
      throw new Error(
        'Roster push query may not contain more than one "item" element.'
      );
    }
    if (items.length === 0) {
      log.warn(iq);
  ...
  updateContact(item) {
    ...
    const groups = Array.from(item.getElementsByTagName('group')).map(
      (e) => e.textContent
    );
    if (!contact) {
      if (
        (subscription === 'none' && ask === null) ||
        subscription === 'remove'
      ) {
  ...
  createRequestingContact(presence) {
    ...
    const nickname =
      sizzle(`nick[xmlns="${Strophe.NS.NICK}"]`, presence).pop()?.textContent ||
      null;
    const user_data = {
      jid: bare_jid,
      subscription: 'none',
