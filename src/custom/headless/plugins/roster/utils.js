...
export function updateUnreadCounter(chatbox) {
  const contact =
    _converse.roster && _converse.roster.findWhere({ jid: chatbox.get('jid') });
...
export function contactsComparator(contact1, contact2) {
  ...
  if (_converse.STATUS_WEIGHTS[status1] === _converse.STATUS_WEIGHTS[status2]) {
    ...
    return _converse.STATUS_WEIGHTS[status1] < _converse.STATUS_WEIGHTS[status2]
      ? -1
      : 1;
  }
}
...
export function groupsComparator(a, b) {
  ...
  if (!a_is_special && !b_is_special) {
    ...
    return WEIGHTS[a_header] < WEIGHTS[b]
      ? -1
      : WEIGHTS[a_header] > WEIGHTS[b]
      ? 1
      : 0;
  } else if (a_is_special && !b_is_special) {
    const b_header = _converse.HEADER_CURRENT_CONTACTS;
    return WEIGHTS[a] < WEIGHTS[b_header]
      ? -1
      : WEIGHTS[a] > WEIGHTS[b_header]
      ? 1
      : 0;
  }
}
