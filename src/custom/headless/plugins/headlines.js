...
import { isHeadline, isServerMessage } from '@converse/staytus/shared/parsers';
  ...
  initialize() {
    ...
    _converse.HeadlinesBox = _converse.ChatBox.extend({
      defaults() {
        return {
          bookmarked: false,
          hidden: ['mobile', 'fullscreen'].includes(
    ...
    function registerHeadlineHandler() {
      _converse.connection.addHandler(
        (message) => onHeadlineMessage(message) || true,
        null,
        'message'
      );
    }
      ...
      headlines: {
        ...
        async get(jids, attrs = {}, create = false) {
          async function _get(jid) {
            ...
            if (!model && create) {
              model = await api.chatboxes.create(
                jid,
                attrs,
                _converse.HeadlinesBox
              );
            } else {
              model =
                model && model.get('type') === _converse.HEADLINES_TYPE
                  ? model
                  : null;
          ...
          if (jids === undefined) {
            ...
            return chats.filter(
              (c) => c.get('type') === _converse.HEADLINES_TYPE
            );
