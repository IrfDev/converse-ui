...
converse.plugins.add('converse-status', {
  initialize() {
    ...
    _converse.XMPPStatus = Model.extend({
      ...
      constructPresence(type, to = null, status_message) {
        type =
          typeof type === 'string'
            ? type
            : this.get('status') || api.settings.get('default_state');
        status_message =
          typeof status_message === 'string'
            ? status_message
            : this.get('status_message');
        let presence;
        const attrs = { to };
        ...
        presence
          .c('priority')
          .t(isNaN(Number(priority)) ? 0 : priority)
          .up();
        if (_converse.idle) {
          const idle_since = new Date();
          idle_since.setSeconds(
            idle_since.getSeconds() - _converse.idle_seconds
          );
          presence.c('idle', {
    ...
    _converse.onEverySecond = function () {
      ...
      if (
        api.settings.get('csi_waiting_time') > 0 &&
    ...
    _converse.registerIntervalHandler = function () {
      ...
      _converse.everySecondTrigger = window.setInterval(
    ...
    function initStatus(reconnecting) {
      ...
      if (reconnecting) {
        ...
        _converse.xmppstatus.browserStorage = _converse.createStore(
          id,
          'session'
        );
