...
import { converse } from '@converse/staytus/core';
...
function generateVerificationString(_converse) {
  ...
  let S = identities.reduce(
    (result, id) =>
      `${result}${id.category}/${id.type}/${id?.lang ?? ''}/${id.name}<`,
    ''
  );
  features.sort();
...
converse.plugins.add('converse-caps', {
  overrides: {
    ...
    XMPPStatus: {
      constructPresence() {
        const presence = this.__super__.constructPresence.apply(
          this,
          arguments
        );
