...
function handleAck(el) {
  ...
  const last_known_handled = _converse.session.get(
...
function stanzaHandler(el) {
  if (_converse.session.get('smacks_enabled')) {
    if (
      u.isTagEqual(el, 'iq') ||
      u.isTagEqual(el, 'presence') ||
      u.isTagEqual(el, 'message')
    ) {
      const h = _converse.session.get('num_stanzas_handled');
...
function initSessionData() {
  _converse.session.save({
    ...
    num_stanzas_handled_by_server:
      _converse.session.get('num_stanzas_handled_by_server') || 0,
    num_stanzas_since_last_ack:
      _converse.session.get('num_stanzas_since_last_ack') || 0,
...
function onFailedStanza(el) {
  if (el.querySelector('item-not-found')) {
    ...
    log.warn(
      'Could not resume previous SMACKS session, session id not found. ' +
...
function resendUnackedStanzas() {
  ...
  //
  // Two possible solutions are running @converse/staytus as a
...
async function sendResumeStanza() {
  ...
  _converse.connection._addSysHandler(
    (el) => promise.resolve(onResumedStanza(el)),
    Strophe.NS.SM,
    'resumed'
  );
  _converse.connection._addSysHandler(
    (el) => promise.resolve(onFailedStanza(el)),
    Strophe.NS.SM,
    'failed'
  );
  ...
  const stanza = u.toStanza(
    `<resume xmlns="${Strophe.NS.SM}" h="${h}" previd="${previous_id}"/>`
  );
  api.send(stanza);
...
async function sendEnableStanza() {
  if (
    !api.settings.get('enable_smacks') ||
    _converse.session.get('smacks_enabled')
  ) {
    return;
  }
  if (await isStreamManagementSupported()) {
    const promise = u.getResolveablePromise();
    _converse.connection._addSysHandler(
      (el) => promise.resolve(saveSessionData(el)),
      Strophe.NS.SM,
      'enabled'
    );
    _converse.connection._addSysHandler(
    ...
    const stanza = u.toStanza(
      `<enable xmlns="${Strophe.NS.SM}" resume="${resume}"/>`
    );
    api.send(stanza);
    _converse.connection.flush();
    await promise;
