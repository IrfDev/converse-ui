...
const ChatBox = ModelWithContact.extend({
  ...
  afterMessagesFetched(messages) {
    this.most_recent_cached_message = messages
      ? this.getMostRecentMessage(messages)
      : null;
    /**
     * Triggered whenever a `_converse.ChatBox` instance has fetched its messages from
     * `sessionStorage` but **NOT** from the server.
  ...
  validate(attrs) {
    ...
    const room_jids = api.settings
      .get('auto_join_rooms')
      .map((s) => (isObject(s) ? s.jid : s));
    const auto_join = api.settings
      .get('auto_join_private_chats')
      .concat(room_jids);
    if (
  ...
  getMessageReferencedByError(attrs) {
    ...
    return (
      ...
      this.messages.models.find((m) =>
        [m.get('msgid'), m.get('retraction_id')].includes(id)
      )
    );
  },
  ...
  getDuplicateMessage(attrs) {
    ...
    return msgs.find((m) =>
      queries.reduce((out, q) => out || isMatch(m.attributes, q), false)
    );
  },
  ...
  sendMarkerForLastMessage(type = 'displayed', force = false) {
    ...
    msgs.reverse();
    const msg = msgs.find(
      (m) => m.get('sender') === 'them' && (force || m.get('is_markable'))
    );
  ...
  createMessageStanza(message) {
    ...
    if (message.get('oob_url')) {
      stanza
        .c('x', { xmlns: Strophe.NS.OUTOFBAND })
        .c('url')
        .t(message.get('oob_url'))
        .root();
    }
    ...
    if (message.get('origin_id')) {
      stanza
        .c('origin-id', { xmlns: Strophe.NS.SID, id: message.get('origin_id') })
        .root();
    }
    return stanza;
  },
  ...
  setEditable(attrs, send_time) {
    ...
    if (api.settings.get('allow_message_corrections') === 'all') {
      ...
      api.settings.get('allow_message_corrections') === 'last' &&
    ...
    if (
      ...
      api.send(
        $msg({
          ...
          .c(this.get('chat_state'), { xmlns: Strophe.NS.CHATSTATES })
          .up()
  ...
  async sendFiles(files) {
    const { __ } = _converse;
    const result = await api.disco.features.get(
      Strophe.NS.HTTPUPLOAD,
      _converse.domain
    );
    ...
    const data = item.dataforms
      ...
      .pop();
    const max_file_size = window.parseInt(
      (data?.attributes || {})['max-file-size']?.value
    );
    ...
    Array.from(files).forEach(async (file) => {
      if (
        !window.isNaN(max_file_size) &&
        window.parseInt(file.size) > max_file_size
      ) {
