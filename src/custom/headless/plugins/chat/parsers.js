...
export async function parseMessage(stanza, _converse) {
  ...
  if (
    api.settings.get('filter_by_resource') &&
    to_resource &&
    to_resource !== _converse.resource
  ) {
  ...
  if (isCarbon(stanza)) {
    if (from_jid === _converse.bare_jid) {
      ...
      return new StanzaParseError(
        `Rejecting carbon from invalid JID ${to_jid}`,
        stanza
      );
    }
  }
  ...
  if (is_archived) {
    if (from_jid === _converse.bare_jid) {
      ...
      return new StanzaParseError(
        `Invalid Stanza: alleged MAM message from ${stanza.getAttribute(
          'from'
        )}`,
        stanza
      );
  ...
  if (attrs.is_archived) {
    ...
    if (from && from !== _converse.bare_jid) {
      return new StanzaParseError(
        `Invalid Stanza: Forged MAM message from ${from}`,
        stanza
      );
    }
  }
  ...
  attrs['id'] =
    attrs['origin_id'] || attrs[`stanza_id ${attrs.from}`] || u.getUniqueId();
