...
import { isServerMessage } from '@converse/staytus/shared/parsers';
  ...
  initialize() {
    ...
    _converse.handleMessageStanza = async function (stanza) {
      if (isServerMessage(stanza)) {
        ...
        return log.info(
          `handleMessageStanza: Ignoring incoming server message from JID: ${from}`
        );
      }
      ...
      const has_body = !!sizzle(
        `body, encrypted[xmlns="${Strophe.NS.OMEMO}"]`,
        stanza
      ).length;
      const chatbox = await api.chats.get(
        attrs.contact_jid,
        { nickname: attrs.nick },
        has_body
      );
    ...
    function registerMessageHandlers() {
      _converse.connection.addHandler(
        (stanza) => {
          if (
            sizzle(`message > result[xmlns="${Strophe.NS.MAM}"]`, stanza).pop()
          ) {
    ...
    api.listen.on('clearSession', async () => {
      if (_converse.shouldClearCache()) {
        await Promise.all(
          _converse.chatboxes.map(
            (c) => c.messages && c.messages.clearStore({ silent: true })
          )
        );
