...
import { getAttributes } from '@converse/staytus/shared/parsers';
...
function parseForCommands(stanza) {
  const items = sizzle(
    `query[xmlns="${Strophe.NS.DISCO_ITEMS}"][node="${Strophe.NS.ADHOC}"] item`,
    stanza
  );
...
const adhoc_api = {
  ...
  adhoc: {
    ...
    async getCommands(to_jid) {
      let commands = [];
      try {
        commands = parseForCommands(
          await api.disco.items(to_jid, Strophe.NS.ADHOC)
        );
      } catch (e) {
        if (e === null) {
          log.error(
            `Error: timeout while fetching ad-hoc commands for ${to_jid}`
          );
        } else {
