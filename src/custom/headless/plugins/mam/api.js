import { RSM } from '@converse/staytus/shared/rsm';
  ...
  archive: {
    ...
    async query(options) {
      if (!api.connection.connected()) {
        throw new Error(
          "Can't call `api.archive.query` before having established an XMPP session"
        );
      }
      ...
      if (!supported) {
        log.warn(
          `Did not fetch MAM archive for ${jid} because it doesn't support ${NS.MAM}`
        );
        return { messages: [] };
      }
      ...
      if (options) {
        stanza
          .c('x', { xmlns: NS.XFORM, type: 'submit' })
        ...
        ['start', 'end'].forEach((t) => {
          if (options[t]) {
            ...
            if (date.isValid()) {
              ...
              throw new TypeError(
                `archive.query: invalid date provided for: ${t}`
              );
            }
          }
        });
      ...
      const message_handler = _converse.connection.addHandler((stanza) => {
        ...
        if (options.groupchat) {
          if (from !== options['with']) {
            log.warn(
              `Ignoring alleged groupchat MAM message from ${stanza.getAttribute(
                'from'
              )}`
            );
            return true;
          ...
          log.warn(
            `Ignoring alleged MAM message from ${stanza.getAttribute('from')}`
          );
          return true;
        }
      ...
      let error;
      const iq_result = await api.sendIQ(
        stanza,
        api.settings.get('message_archiving_timeout'),
        false
      );
      if (iq_result === null) {
        const { __ } = _converse;
      ...
      let rsm;
      const fin =
        iq_result && sizzle(`fin[xmlns="${NS.MAM}"]`, iq_result).pop();
