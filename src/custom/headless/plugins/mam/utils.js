...
import { parseMUCMessage } from '@converse/staytus/plugins/muc/parsers';
import { parseMessage } from '@converse/staytus/plugins/chat/parsers';
import { _converse, api, converse } from '@converse/staytus/core';
...
export function onMAMError(iq) {
  if (iq?.querySelectorAll('feature-not-implemented').length) {
    log.warn(
      `Message Archive Management (XEP-0313) not supported by ${iq.getAttribute(
        'from'
      )}`
    );
  } else {
    log.error(
      `Error while trying to set archiving preferences for ${iq.getAttribute(
        'from'
      )}.`
    );
    log.error(iq);
...
export function onMAMPreferences(iq, feature) {
  ...
  if (default_pref !== api.settings.get('message_archiving')) {
    ...
    Array.from(preference.children).forEach((child) =>
    ...
    api
      ...
      .then(() =>
        feature.save({
          preferences: { default: api.settings.get('message_archiving') },
        })
      )
...
export function getMAMPrefsFromFeature(feature) {
  ...
  if (
    feature.get('var') !== NS.MAM ||
    api.settings.get('message_archiving') === undefined
  ) {
    return;
  }
  if (prefs['default'] !== api.settings.get('message_archiving')) {
    api
      .sendIQ($iq({ type: 'get' }).c('prefs', { xmlns: NS.MAM }))
...
export async function handleMAMResult(
  model,
  result,
  query,
  options,
  ...
  if (result.error) {
    ...
    api.listen.once(event_id, () =>
      fetchArchivedMessages(model, options, should_page)
    );
