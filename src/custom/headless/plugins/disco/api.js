import { _converse, api, converse } from '@converse/staytus/core.js';
  ...
  disco: {
    ...
    stream: {
      ...
      async getFeature(name, xmlns) {
        ...
        if (
          _converse.stream_features === undefined &&
      ...
      identities: {
        ...
        add(category, type, name, lang) {
          for (var i = 0; i < _converse.disco._identities.length; i++) {
            if (
              _converse.disco._identities[i].category == category &&
          ...
          _converse.disco._identities.push({
            category: category,
            type: type,
            name: name,
            lang: lang,
      ...
      features: {
        ...
        add(name) {
          for (var i = 0; i < _converse.disco._features.length; i++) {
            if (_converse.disco._features[i] == name) {
              return false;
            }
          }
    ...
    entities: {
      ...
      async get(jid, create = false) {
        ...
        if (
          _converse.disco_entities === undefined &&
          !api.connection.connected()
        ) {
          // Happens during tests when disco lookups happen asynchronously after teardown.
    ...
    features: {
      ...
      async get(feature, jid) {
        ...
        const promises = [
          ...entity.items.map((i) => i.hasFeature(feature)),
