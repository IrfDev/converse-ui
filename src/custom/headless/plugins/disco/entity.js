...
const DiscoEntity = Model.extend({
  ...
  onDiscoItems(stanza) {
    sizzle(`query[xmlns="${Strophe.NS.DISCO_ITEMS}"] item`, stanza).forEach(
      (item) => {
  ...
  onInfo(stanza) {
    ...
    sizzle(`x[type="result"][xmlns="${Strophe.NS.XFORM}"]`, stanza).forEach(
      (form) => {
        const data = {};
    ...
    sizzle('x[type="result"][xmlns="jabber:x:data"] field', stanza).forEach(
      (field) => {
        this.fields.create({
