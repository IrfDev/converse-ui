...
function onDiscoInfoRequest(stanza) {
  ...
  _converse.disco._features.forEach((feature) =>
...
export async function initializeDisco() {
  ...
  _converse.disco_entities.browserStorage = _converse.createStore(
    id,
    'session'
  );
...
export function initStreamFeatures() {
  ...
  if (!_converse.stream_features) {
    ...
    _converse.stream_features.browserStorage = _converse.createStore(
      id,
      'session'
    );
  }
