  ...
  initialize() {
    ...
    api.listen.on('userSessionInitialized', async () => {
      ...
      if (
        ...
        await new Promise((success, error) =>
          _converse.stream_features.fetch({ success, error })
        );
        notifyStreamFeaturesAdded();
      }
    });
    ...
    api.listen.on('clearSession', () => {
      if (_converse.shouldClearCache() && _converse.disco_entities) {
        Array.from(_converse.disco_entities.models).forEach((e) =>
          e.features.clearStore()
        );
        Array.from(_converse.disco_entities.models).forEach((e) =>
          e.identities.clearStore()
        );
        Array.from(_converse.disco_entities.models).forEach((e) =>
          e.dataforms.clearStore()
        );
        Array.from(_converse.disco_entities.models).forEach((e) =>
          e.fields.clearStore()
        );
