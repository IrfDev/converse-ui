...
u.isSameBareJID = function (jid1, jid2) {
  ...
  return (
    Strophe.getBareJidFromJid(jid1).toLowerCase() ===
...
u.isSameDomain = function (jid1, jid2) {
  ...
  return (
    Strophe.getDomainFromJid(jid1).toLowerCase() ===
...
u.isForbiddenError = function (stanza) {
  ...
  return (
    sizzle(
      `error[type="auth"] forbidden[xmlns="${Strophe.NS.STANZAS}"]`,
      stanza
    ).length > 0
  );
};
...
u.isServiceUnavailableError = function (stanza) {
  ...
  return (
    sizzle(
      `error[type="cancel"] service-unavailable[xmlns="${Strophe.NS.STANZAS}"]`,
      stanza
    ).length > 0
  );
};
...
u.queryChildren = function (el, selector) {
  return Array.from(el.childNodes).filter((el) =>
    u.matchesSelector(el, selector)
  );
};

u.contains = function (attr, query) {
  const checker = (item, key) =>
    item.get(key).toLowerCase().includes(query.toLowerCase());
  return function (item) {
    if (typeof attr === 'object') {
      return Object.keys(attr).reduce(
        (acc, k) => acc || checker(item, k),
        false
      );
...
u.rootContains = function (root, el) {
  ...
  return root.contains
    ? root.contains(el)
    : window.HTMLElement.prototype.contains.call(root, el);
};
...
u.isMentionBoundary = (s) =>
  s !== '@' && RegExp(`(\\p{Z}|\\p{P})`, 'u').test(s);

u.replaceCurrentWord = function (input, new_value) {
  const caret = input.selectionEnd || undefined;
  const current_word = last(input.value.slice(0, caret).split(/\s/));
  const value = input.value;
  const mention_boundary = u.isMentionBoundary(current_word[0])
    ? current_word[0]
    : '';
  input.value =
    value.slice(0, caret - current_word.length) +
    mention_boundary +
    `${new_value} ` +
    value.slice(caret);
